...
class Turry(Module):
    ...
    async def process_message(self, message, client=None):
        ...
        if hasattr(message.channel, "name") and message.channel.name == "seed":
            ...
            if command == "!new":
                ...
                if self.adventure_id:
                    message = await self.process_game_next(prompt, self.adventure_id, self.last_seen_id)
                    if message:
                        response = message[0]
    ...
    async def process_game_next(self, text, adventure_id, last_seen_id):
        ws = self.ws
        print("adventure id:")
        print(adventure_id)
        ...
        # send message
        await ws.send('{"id":"2","type":"start","payload":{"variables":{"input":{"publicId":"' + str(adventure_id) + '","type":"story","text":' + json.dumps(text) + ',"choicesMode":false}},"extensions":{},"query":"mutation ($input: ActionInput) { addAction(input: $input) { time message __typename }}","auth":{"token":"hello"}}}')

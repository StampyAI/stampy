...
class Turry(Module):
    ...
    def can_process_message(self, message, client=None):
        if hasattr(message.channel, "name") and (
            message.channel.name == "seed" or message.channel.name == "seed-test"
        ):
            return 7, ""
        return 0, ""
    ...
    async def process_message(self, message, client=None):
        ...
        if hasattr(message.channel, "name") and (
            ...
            if command == "!new":
                ...
                await message.channel.trigger_typing()
                self.ws = await self.get_initialized_websocket()
                message = await self.process_game_next(
                    "do", prompt, self.adventure_id, self.last_seen_id
                )
                if message:
                    response = message[0]
                ...
                self.ws = await self.get_initialized_websocket()
                message = await self.process_game_next(
                    "say", prompt, self.adventure_id, self.last_seen_id
                )
                if message:
                    response = message[0]
                    self.last_seen_id = message[1]
                ...
                self.ws = await self.get_initialized_websocket()
                message = await self.process_game_next(
                    "story", prompt, self.adventure_id, self.last_seen_id
                )
                if message:
                    response = message[0]
                    self.last_seen_id = message[1]
    ...
    async def process_game_revert(self, adventure_id):
        ...
        await ws.send(
            ...
            + str(adventure_id)
            + '","type":"undo","choicesMode":false}},"extensions":{},"query":"mutation ($input: ActionInput) { addAction(input: $input) { time message __typename }}","auth":{"token":"hello"}}}'
        )

...
#     def __init__(self):
#         super().__init__()
#         re_intro = r"([wW]hat(’|'| i)?s|([Cc]an|[Mm]ay) (we|[iI]) (have|get)|[Ll]et[’']?s have|[gG]ive us)?"
#         re_another_next_q = r"( ?[Aa](nother)?|( the)? ?[nN]ext)"
#         re_q_type = r"{question_type}"
#         re_please = r",?( please)?\??"
#         re_have_you_got_more_q = r"(([Dd]o you have |[Hh]ave you )[Gg]ot)?( ?[Aa]ny( more| other)?| another) {question_type}s?( for us)?\??)!?"
#         re_generic_q = "(" + re_intro + re_another_next_q + " " + re_q_type + re_please + "|" + "(" + re_have_you_got_more_q + ")" + ")"

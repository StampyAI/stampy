...
class QQManager(Module):
    ...
    def process_message(self, message):
        if self.is_at_me(message):
            ...
            if self.question_count_regex.match(text):
                ...
                return Response(
                    confidence=10, callback=self.post_question, args=[message], kwargs={"wiki_question_bias": 1}
    ...
    async def post_question(self, message, wiki_question_bias=SemanticWiki.default_wiki_question_percent_bias):
        ...
        if result:
            return Response(confidence=10, text=result, why="%s asked for a question to answer" % message.author.name)
        else:
            return Response(
                confidence=8,
    ...
    def test_cases(self):
        return [
            ...
            self.create_integration_test(
                question="what is the next questions in the queue", expected_response=self.EMPTY_QUEUE_MESSAGE,

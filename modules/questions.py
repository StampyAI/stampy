...
class QQManager(Module):
    ...
    def __init__(self):
        ...
        self.re_nextq = re.compile(
            r"""(([wW]hat(’|'| i)?s|([Cc]an|[Mm]ay) (we|[iI]) (have|get)|[Ll]et(’|')?s have|[gG]ive us)?( ?[Aa](nother)?|( the)? ?[nN]ext) question,?( please)?\??|
        ...
        if self.isatme(message):
            ...
            if re.match(
                ...
                return (
                    9,
                    "",
                )  # Popping a question off the stack modifies things, so just return a "yes, we can handle this" and let processMessage do it

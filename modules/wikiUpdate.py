...
class WikiUpdate(Module):
    ...
    def __init__(self):
        ...
        # FIXME: for some reason, an partial asynchronous class function is not detected as async by iscoroutinefunction
        # FIXME: so the values of command can either be a partial class function or async partial function, but not both
        # FIXME: a workaround: define a function outside of the class, but gives it a parameter fake_self
        ...
        self.command_dict = {
            ...
            "Rejected": {
                "re": re.compile(
                    tag_this_as_regex
                    + r" '?rejected'?|reject (that|this)"
                    # is it confusing that |option2 matches option2 alone, and not tag_this_as_regex+option2?
                ),

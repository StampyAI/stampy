	...
	def sortbyrelevance(self, videos, searchstring, reverse=False):
		...
		for video in videos:
			...
			for keyword in keywords:
				...
				video.score += 3.0 * video.title.lower().count(keyword.lower()) / (len(video.title) + 1)
				video.score += 1.0 * video.description.lower().count(keyword.lower()) / (len(video.description) + 1)
				video.score += 1.0 * video.text.lower().count(keyword.lower()) / (len(video.text) + 1)
				# print(video.score)

...
async def on_message(message):
    ...
    if message.content == "reply test":
        if message.reference:
            ...
            response += (
                'which should be taken as an answer to the question at: "%s"' % replyURL
            )
        else:
    ...
    if (
        ...
        await message.channel.send(
            "[Invite Roles Reset for %s users]" % resetuserscount
        )
        return
    ...
    if not result:  # no results from the modules, try the sentience core
        try:
            ...
            if hasattr(message.channel, "name") and message.channel.name in (
                ...
                try:
                    errortype = sentience.dereference(
                        "{{$errorType}}"
                    )  # grab a random error type from the factoid db
                except:
...
async def on_socket_raw_receive(msg):
    ...
    if qq:
        ...
        if (now - utils.lastqaskts) > qaskcooldown:
            if (
                not utils.lastmessagewasYTquestion
            ...
            print(
                ...
                % (len(qq), str(qaskcooldown - (now - utils.lastqaskts)))
            )
            return
...
if __name__ == "__main__":
    ...
    utils.lastmessagewasYTquestion = (
